...
function normalizeFilters(params = {}) {
  ...
  return {
    fir_year: clean(params.fir_year ?? params.year ?? params.firYear ?? ""),
    fir_month: clean(params.fir_month ?? params.month ?? params.firMonth ?? ""),
    district: pick("district"),
    police_station: clean(params.police_station ?? params.policeStation ?? ""),
    crime_type: clean(params.crime_type ?? params.crime_group ?? params.crimeType ?? ""),
    crime_subtype: pick("crime_subtype"),
    severity: clean(params.severity ?? params.fir_type ?? params.firType ?? ""),
    fir_stage: clean(params.fir_stage ?? params.status ?? params.firStage ?? ""),
    complaint_mode: clean(params.complaint_mode ?? params.complaintMode ?? ""),
...
function applyFilters(records, params = {}) {
  ...
  const normalizeText = (value) => clean(safeDecode(value)).toLowerCase().replace(/\s+/g, " ");
  const matchesText = (actual, expected) => normalizeText(actual) === normalizeText(expected);
  const matchesNumber = (actual, expected) => {
    const actualNumber = Number(clean(actual));
    const expectedNumber = Number(clean(expected));
  ...
  return records.filter((record) => {
    if (isActiveFilter(filters.fir_year) && !matchesNumber(record.fir_year, filters.fir_year)) return false;
    if (isActiveFilter(filters.fir_month) && !matchesNumber(record.fir_month, filters.fir_month)) return false;
    if (isActiveFilter(filters.district) && !matchesText(record.district, filters.district)) return false;
    if (isActiveFilter(filters.police_station) && !matchesText(record.police_station, filters.police_station)) return false;
    if (isActiveFilter(filters.crime_type) && !matchesText(record.crime_type, filters.crime_type)) return false;
    if (isActiveFilter(filters.crime_subtype) && !matchesText(record.crime_subtype, filters.crime_subtype)) return false;
    if (isActiveFilter(filters.severity) && !matchesText(record.severity, filters.severity) && !matchesText(record.severity_original, filters.severity)) return false;
    if (isActiveFilter(filters.fir_stage) && !matchesText(record.fir_stage, filters.fir_stage)) return false;
    if (isActiveFilter(filters.complaint_mode) && !matchesText(record.complaint_mode, filters.complaint_mode)) return false;
    if (isActiveFilter(filters.dataset_id) && !matchesText(record.dataset_id, filters.dataset_id)) return false;
    return true;
  });
}
